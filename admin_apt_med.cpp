...
admin_apt_med::admin_apt_med(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::admin_apt_med)
{
    ui->setupUi(this);
...
admin_apt_med::~admin_apt_med()
{
    delete ui;
}


void admin_apt_med::on_submitbtn_clicked()
{
    ...
    QSqlQuery qry_qn;
    qry_qn.prepare("insert into question_apt_med(qn,qn_no,choice_1,choice_2,choice_3,choice_4,crt_choice)values ('"+qn_editor+"','"+qn_number+"','"+choice_1+"','"+choice_2+"','"+choice_3+"','"+choice_4+"','"+crt_choice+"')");
    if(qry_qn.exec())
    {
...
void admin_apt_med::on_quit_clicked()
{
    QApplication::quit();
}
